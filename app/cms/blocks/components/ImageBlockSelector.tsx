      ...
      <div className="mt-1 p-3 border rounded-md bg-muted/30 min-h-[120px] flex flex-col items-center justify-center">
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          ...
          <DialogContent className="sm:max-w-[650px] md:max-w-[800px] lg:max-w-[1000px] max-h-[80vh] flex flex-col">
            {/* ... (Modal content remains largely the same, ensure handleSelectMediaFromLibrary sets object_key) ... */}
            <DialogHeader><DialogTitle>Media Library</DialogTitle></DialogHeader>
            <div className="relative mb-4">
                <Input type="search" placeholder="Search..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground"/>
            </div>
            {isLoadingLibrary ? <div className="flex-grow flex items-center justify-center"><p>Loading media...</p></div>
            : mediaLibrary.length === 0 ? <div className="flex-grow flex items-center justify-center"><p>No media found.</p></div>
            : (
